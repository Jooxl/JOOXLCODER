...
function updateEffectsTheme() {
    currentPrimaryRGB = getComputedStyle(document.body).getPropertyValue('--primary-rgb').trim() || '255, 85, 0';
}
setTimeout(updateEffectsTheme, 100);
window.addEventListener('themeChanged', updateEffectsTheme);
...
class CircuitLine {
    ...
    draw(ctx) {
        ...
        ctx.beginPath(); ctx.moveTo(this.x, this.y); ctx.lineTo(this.x + this.dx * Math.min(this.progress, 1), this.y + this.dy * Math.min(this.progress, 1));
...
class LineHighlight {
    constructor(y, width) {
        this.y = y - 5; // Offset to match text baseline roughly
        this.width = width;
        this.life = 1;
    }
    update() {
        this.life -= 0.06; // Fades out in roughly ~16 frames (250ms)
    }
    draw(ctx) {
...
class Spark {
    constructor(x, y, color = null, vx, vy) {
        this.x = x; this.y = y; this.life = 1; this.color = color || `rgba(${currentPrimaryRGB}, 1)`;
    ...
    draw(ctx) { ctx.fillStyle = this.color.replace('1)', `${this.life})`); ctx.beginPath(); ctx.arc(this.x, this.y, this.size, 0, Math.PI*2); ctx.fill(); }
}

class Shockwave {
    constructor(x, y, color) { this.x = x; this.y = y; this.life = 1; this.radius = 0; this.color = color; }
    update() { this.life -= 0.05; this.radius += 5; }
    draw(ctx) { ctx.strokeStyle = this.color.replace('1)', `${this.life})`); ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(this.x, this.y, this.radius, 0, Math.PI*2); ctx.stroke(); }
}
...
class Spiral {
    ...
    draw(ctx) {
        ...
        ctx.beginPath(); ctx.arc(this.x + Math.cos(this.angle)*this.radius, this.y + Math.sin(this.angle)*this.radius, 3, 0, Math.PI*2); ctx.fill();
    }
}
...
class FracturingGrid {
    ...
    draw(ctx) {
        ctx.strokeStyle = this.color.replace('1)', `${this.life})`); ctx.lineWidth = 1;
        ctx.strokeRect(this.x - this.size, this.y - this.size, this.size*2, this.size*2);
        ctx.beginPath(); ctx.moveTo(this.x - this.size, this.y); ctx.lineTo(this.x + this.size, this.y); ctx.moveTo(this.x, this.y - this.size); ctx.lineTo(this.x, this.y + this.size); ctx.stroke();
    }
}

class GlowingRune {
    constructor(x, y, color) { this.x = x; this.y = y; this.life = 1; this.color = color; this.char = String.fromCharCode(0x30A0 + Math.random() * 96); this.vy = -2; }
    update() { this.life -= 0.02; this.y += this.vy; }
    draw(ctx) {
        ctx.fillStyle = this.color.replace('1)', `${this.life})`); ctx.font = "20px 'Fira Code'";
        ctx.fillText(this.char, this.x - 10, this.y);
    }
}

class PolygonExplosion {
    constructor(x, y, color, sides) { this.x = x; this.y = y; this.life = 1; this.color = color; this.radius = 5; this.sides = sides; this.rot = 0; }
    update() { this.life -= 0.05; this.radius += 3; this.rot += 0.1; }
    draw(ctx) {
        ctx.strokeStyle = this.color.replace('1)', `${this.life})`); ctx.lineWidth = 2; ctx.save(); ctx.translate(this.x, this.y); ctx.rotate(this.rot);
        ctx.beginPath();
        for (let i = 0; i <= this.sides; i++) { const ang = i * 2 * Math.PI / this.sides; i === 0 ? ctx.moveTo(this.radius * Math.cos(ang), this.radius * Math.sin(ang)) : ctx.lineTo(this.radius * Math.cos(ang), this.radius * Math.sin(ang)); }
...
function spawnEffect(char, x, y, cColor = null) {
    ...
    if (char === '@') activeEffects.push(new Spiral(x, y, color), new Spiral(x, y, color));
    ...
    else if (char === '^') activeEffects.push(new PolygonExplosion(x, y, color, 3)); // Triangle
    ...
    else if (char === '{' || char === '}') activeEffects.push(new PolygonExplosion(x, y, color, 4)); // Square
        ...
        // Standard Letter/Number
        for(let i=0; i<3; i++) activeEffects.push(new Spark(x, y, color));
    }

    if (activeEffects.length > MAX_PARTICLES) activeEffects.splice(0, activeEffects.length - MAX_PARTICLES);
}
...
function animateAll() {
    ...
    for (let i = circuitLines.length - 1; i >= 0; i--) {
        circuitLines[i].update(); circuitLines[i].draw(circuitCtx);
